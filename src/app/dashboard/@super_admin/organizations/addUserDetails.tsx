...
export default function AddUser({ setOpenAddUser, selectedOrganization }: { setOpenAddUser: Dispatch<SetStateAction<boolean>>, selectedOrganization: Organization }) {
    ...
    const onSave = useCallback(async ({ email, city, country, department, grade, group, name, role, status, title }: any) => {
        ...
        try {
            const { data, error } = await adminSupabase.auth.admin.inviteUserByEmail(email, {
                redirectTo: window?.location?.protocol + '//' + selectedOrganization.domain + "/welcome"
            })
            ...
            let { error: addUserError } = await supabase
                .rpc('add_user_super', {
                    _organization_id: selectedOrganization.id,
                    _is_active: status === 'active', // Replaced status with is_active
                    _user_city: city,
                    _user_country: country,
                    _user_department: department,
                    _user_email: email,
                    _user_group_id: group || null,
                    _user_id: data.user.id,
                    _user_job_grade: grade,
                    _user_job_title: title,
                    _user_name: name,
                    _user_role: role,
                    _domain: selectedOrganization.domain,
                    _lang: "en"
                });
